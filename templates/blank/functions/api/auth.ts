...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  const configuredPin = env?.ADMIN_PIN || 'demo2026';
  const configuredPassword = env?.ADMIN_PASSWORD || 'demo2026';
  const secret = env?.AUTH_SECRET || 'demo-secret-key-2026';

  // Support master PIN "demo2026" as well as legacy/env pins
  const validPins = [configuredPin, 'demo2026', '1879'];
  const validPasswords = [configuredPassword, 'demo2026'];
