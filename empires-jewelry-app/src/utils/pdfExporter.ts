...
export function exportWishlistToPDF(favoriteProducts: Product[]): void {
  ...
  const htmlContent = `
    ...
    <head>
      <meta charset="UTF-8">
      <title>Catálogo Personalizado VIP — Empires Jewelry</title>
      <style>
    ...
    <body>
      <div class="no-print" style="margin-bottom: 20px; text-align: right;">
        <button onclick="window.print()" style="padding: 10px 20px; background: #1A1817; color: white; border: none; border-radius: 4px; cursor: pointer; font-family: Montserrat; font-size: 12px; text-transform: uppercase; letter-spacing: 1px;">
          Imprimir / Guardar como PDF
        </button>
      </div>

      <div class="header">
        <div class="logo">EMPIRES JEWELRY</div>
        <div class="subtitle">Resumen Ejecutivo de Selección Personalizada</div>
      </div>

      <div class="meta-info">
        <span><strong>Fecha de Emisión:</strong> ${currentDate}</span>
        <span><strong>Total Piezas Seleccionadas:</strong> ${favoriteProducts.length}</span>
      </div>
      ...
      <div class="summary-box">
        <div>
          ...
          <div style="font-size: 10px; color: #6B6359;">Cotización sujeto a verificación de disponibilidad por Concierge VIP</div>
        </div>
        <div style="text-align: right;">
          <div style="font-size: 20px; font-weight: 700; color: #C5A059;">${formatPrice(totalUSD, 'USD')}</div>
          <div style="font-size: 11px; color: #6B6359;">~ ${formatCOPApprox(totalUSD)} COP</div>
        </div>
      </div>

      <div class="footer">
        <p>EMPIRES JEWELRY BOUTIQUE &amp; HAUTE JOAILLERIE — Asesoría VIP WhatsApp: +57 (300) 000-0000</p>
        <p>Documento generado automáticamente desde el Portal Oficial de Empires Jewelry.</p>
      </div>
