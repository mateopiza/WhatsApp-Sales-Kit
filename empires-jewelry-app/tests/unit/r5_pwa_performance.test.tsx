...
describe('Tier 1 - R5: PWA Architecture & Performance Utilities', () => {
  it('T1-R5-01: Manifest JSON exists, has standalone mode and brand theme color #F5EDE6', () => {
    const manifestPath = path.resolve(__dirname, '../../public/manifest.json');
    expect(fs.existsSync(manifestPath)).toBe(true);

    const manifestContent = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
    expect(manifestContent.display).toBe('standalone');
    expect(manifestContent.theme_color).toBe('#F5EDE6');
    expect(manifestContent.background_color).toBe('#F5EDE6');
  ...
  it('T1-R5-02: Service Worker sw.js exists and contains CacheFirst and StaleWhileRevalidate strategies', () => {
    ...
    expect(swContent).toContain('empires-static-v1');
    expect(swContent).toContain('empires-shell-v1');
  ...
  it('T1-R5-04: SkeletonImage transitions to full opacity on image load', () => {
    render(
      <SkeletonImage
        src="/assets/products/packaging-box.png"
        alt="Estuche Empires"
        aspectRatio="portrait"
      />
    );

    const img = screen.getByAltText('Estuche Empires');
