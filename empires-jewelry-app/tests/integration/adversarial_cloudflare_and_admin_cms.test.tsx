...
describe('Adversarial Security, Edge Cases & Resilience Suite', () => {
  const secret = 'empires-test-secret-key-2026';
  const env: Env = {
    AUTH_SECRET: secret,
  ...
  describe('Category 1: Cloudflare API Handler Security & Adversarial Payloads', () => {
    ...
    it('JSON-01: Rejects malformed / unparseable JSON in POST /api/auth with 400', async () => {
      const req = new Request('http://localhost/api/auth', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: '{"pin": "empires2026", broken_json: ',
      });
    ...
    it('VAL-03: Rejects missing or blank required fields with 400 Bad Request', async () => {
      const testCases = [
        ...
        { payload: { name: 'Anillo', reference: 'REF-01', category: 'invalida', price: 100 }, expectedErr: 'Categoría' },
        { payload: { name: 'Anillo', reference: 'REF-01', category: 'anillos', collection: '', price: 100 }, expectedErr: 'colección' },
        { payload: { name: 'Anillo', reference: 'REF-01', category: 'anillos', collection: 'Col', price: 100, short_description: 'Corta' }, expectedErr: 'corta' },
        { payload: { name: 'Anillo', reference: 'REF-01', category: 'anillos', collection: 'Col', price: 100, short_description: 'Valida con diez', description: 'Muy corta' }, expectedErr: 'completa' },
      ];
  ...
  describe('Category 2: Admin CMS Edge Cases, Concurrency & Rate Limiting', () => {
    ...
    it('CMS-06: Rate limiting lockout is enforced after exactly 5 invalid PIN attempts', async () => {
      ...
      // Attempt 6 (Even with the REAL PIN 'empires2026') -> Must be blocked!
      let res6 = await act(async () => result.current.login('empires2026'));
    ...
    it('CMS-07: Rate limiting lockout persists across session/provider reloads', async () => {
      ...
      const attempt = await act(async () => result.current.login('empires2026'));
  ...
  describe('Category 3: Multi-Tier Storage Fallback & Offline Resilience', () => {
    ...
    it('OFFLINE-02: Offline getProducts with category/price/search filtering applies locally', async () => {
      ...
      expect(response.success).toBe(true);
      expect(response.meta?.tier).toBe('LOCAL_CACHE');
      expect(response.data!.every((p) => p.category === 'anillos')).toBe(true);
      expect(response.data!.some((p) => p.name.includes('Eternity'))).toBe(true);
    });

    it('OFFLINE-03: Offline getProductById transparently resolves from local cache', async () => {
      vi.spyOn(globalThis, 'fetch').mockRejectedValue(new TypeError('Failed to fetch'));

      const response = await apiService.getProductById('emp-01');
      expect(response.success).toBe(true);
      expect(response.meta?.tier).toBe('LOCAL_CACHE');
      expect(response.data?.id).toBe('emp-01');
    ...
    it('OFFLINE-05: Offline updateProduct updates local cache and returns updated product', async () => {
      ...
      const updateRes = await apiService.updateProduct('emp-01', {
        price: 3890,
        name: 'Gargantilla Solitario Aureum Editada Offline',
      });
      ...
      const localCatalog = apiService.getLocalCatalog();
      const updatedItem = localCatalog.find((p) => p.id === 'emp-01');
      expect(updatedItem?.price).toBe(3890);
      expect(updatedItem?.name).toBe('Gargantilla Solitario Aureum Editada Offline');
    });
    ...
    it('OFFLINE-07: Offline login supports master PINs (empires2026, 1879) and issues mock token', async () => {
      vi.spyOn(globalThis, 'fetch').mockRejectedValue(new TypeError('Failed to fetch'));
    ...
    it('OFFLINE-08: Complete React UI Admin Flow survives total network disconnection without crash', async () => {
      ...
      fireEvent.change(nameInput, { target: { value: 'Anillo Offline Solitario' } });
      ...
      fireEvent.change(shortDescInput, {
        target: { value: 'Anillo creado con red desconectada.' },
      });
      const descInput = screen.getByPlaceholderText(/Historia orfebre detallada, inspiración/i);
      fireEvent.change(descInput, {
        target: { value: 'Elaborado con técnica ancestral y resiliencia offline completa.' },
      });
      ...
      // 5. Piece appears in table
      await waitFor(() => {
        expect(screen.getAllByText(/Anillo Offline Solitario/i).length).toBeGreaterThan(0);
      });

      // 6. Close admin modal -> Piece is displayed in lookbook
