...
describe('Tier 1 - R2: Editorial Home & Navigation', () => {
  it('T1-R2-01: EditorialHero renders luxury badge, brand headline, and Explorar CTA', () => {
    ...
    expect(screen.getByText(/Powered by WhatsApp Sales Kit/i)).toBeInTheDocument();
    expect(
      screen.getByRole('heading', { level: 1, name: /Vende Directo Por WhatsApp/i })
    ).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /Explorar el catálogo/i })).toBeInTheDocument();
  });
  ...
  it('T1-R2-02: Clicking Explorar Piezas invokes smooth scrolling to catalog section', () => {
    ...
    const ctaBtn = screen.getByRole('button', { name: /Explorar el catálogo/i });
