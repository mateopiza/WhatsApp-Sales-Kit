...
describe('Tier 2: Boundary & Corner Cases', () => {
  ...
  it('T2-BC-01: Handles whitespace-only search query without error, returning full catalog', () => {
    ...
    // Displays all 6 demo products found
    expect(screen.getByText(/6 productos encontrados/i)).toBeInTheDocument();
  });
  ...
  it('T2-BC-02: Zero-match filter combination displays elegant empty state with reset button', () => {
    ...
    const input = screen.getByPlaceholderText(/Buscar por nombre/i);
    fireEvent.change(input, { target: { value: 'NonExistentProductItemXYZ' } });
  ...
  it('T2-BC-09: Diacritic-insensitive search matches accented words using unaccented query and vice versa', () => {
    ...
    // Unaccented "edicion" matches the "Edición Limitada" collection/material
    act(() => {
      result.current.setSearchQuery('edicion');
    });
    expect(result.current.filteredProducts.some((p) => p.collection.includes('Edición'))).toBe(true);

    // Accented query "Colección" matches "Colección General"
    act(() => {
      result.current.setSearchQuery('Colección');
    });
    expect(result.current.filteredProducts.length).toBeGreaterThan(0);

    // Unaccented query "Coleccion" matches "Colección General"
    act(() => {
