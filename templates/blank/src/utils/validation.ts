...
export function validateProductForm(
  ...
  if (!data.slug || !slugRegex.test(data.slug.trim())) {
    ...
    if (slugExists) {
      errors.slug = 'Este slug ya está en uso por otro producto del catálogo.';
    }
  }

  // 3. Reference SKU validation
  if (!data.reference || data.reference.trim().length < 2) {
    errors.reference = 'La referencia SKU es obligatoria (ej. REF-0101).';
  } else {
    const refExists = existingProducts.some(
      (p) => p.reference.toUpperCase() === data.reference.trim().toUpperCase() && p.id !== editingId
    );
    if (refExists) {
      errors.reference = 'Esta referencia SKU ya está asignada a otro producto.';
    }
  }

  // 4. Category validation
  if (!data.category || data.category.trim().length < 2) {
    errors.category = 'Seleccione o escriba una categoría.';
  }

  // 5. Collection validation
  if (!data.collection || data.collection.trim().length < 2) {
    errors.collection = 'Especifique la colección a la que pertenece el producto.';
  }
  ...
  if (!data.material) {
    errors.material = 'Seleccione o escriba un material.';
  }
