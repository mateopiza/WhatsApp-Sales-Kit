...
const BRAND_PRESET_IMAGES = [
  { label: 'Hero Collier & Rings', url: '/assets/hero/hero-necklace-rings.png' },
  { label: 'Caja Packaging Lujo', url: '/assets/products/packaging-box.png' },
  { label: 'Bolsa Boutique Empires', url: '/assets/products/packaging-bag.png' },
  { label: 'Etiqueta Certificada Tag', url: '/assets/products/packaging-tag.png' },
  { label: 'Grid Social Lifestyle', url: '/assets/imagery/social-phone-grid.png' },
  { label: 'Papelería & Business Cards', url: '/assets/imagery/business-cards.png' },
];

const DEFAULT_CATEGORIES: { id: Category; label: string }[] = [
  { id: 'anillos', label: 'Anillos' },
  { id: 'collares', label: 'Collares' },
  { id: 'pulseras', label: 'Pulseras' },
  { id: 'aretes', label: 'Aretes' },
  { id: 'alta-joyeria', label: 'Alta Joyería' },
];
...
export const ProductForm: React.FC = () => {
  ...
  const [formData, setFormData] = useState<ProductFormData>(() => {
    ...
    return {
      ...
      images: ['/assets/hero/hero-necklace-rings.png', '/assets/products/packaging-box.png'],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: Product = {
      id: editingProduct?.id || `emp-${Date.now().toString(36)}-${Math.random().toString(36).substring(2, 6)}`,
      ...
      specifications: {
        carats: formData.specifications.carats?.trim() || undefined,
        weight: formData.specifications.weight?.trim() || undefined,
        dimensions: formData.specifications.dimensions?.trim() || undefined,
        stone: formData.specifications.stone?.trim() || undefined,
        purity: formData.specifications.purity?.trim() || undefined,
        closure: formData.specifications.closure?.trim() || undefined,
      },
    };
    ...
    <form onSubmit={handleSubmit} className="space-y-6 animate-fade-in max-w-4xl mx-auto pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-cream p-4 sm:p-5 rounded-md border border-taupe/25 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-display text-xl sm:text-2xl text-ink font-semibold tracking-wide">
              {isEditing ? `Editar Joya: ${editingProduct?.name}` : 'Crear Nueva Joya'}
            </h2>
            <p className="text-xs text-taupe-contrast mt-0.5 font-light">
              {isEditing ? `Referencia SKU: ${editingProduct?.reference}` : 'Complete la ficha técnica y narrativa editorial'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 self-end sm:self-auto">
          ...
          >
            ...
            {isEditing ? 'Guardar Cambios' : 'Publicar Pieza'}
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 border-b border-taupe/25 overflow-x-auto pb-2 scrollbar-none">
        {[
          ...
          { id: 'narrative', label: '3. Narrativa Editorial' },
          { id: 'media', label: '4. Galería & Multimedia' },
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Ej. Gargantilla Solitario Aureum"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-taupe/10">
            <div>
              ...
              <input
                ...
                placeholder="Ej. Pieza Icónica, Nupcial, Alta Demanda"
                className="w-full px-3 py-2 text-xs bg-cream-200/50 border border-taupe/20 rounded-sm focus:outline-none focus:border-gold text-ink"
              />
            </div>
        ...
        <div className="space-y-4 bg-cream p-5 rounded-sm border border-taupe/15 shadow-sm animate-fade-in">
          <div>
            ...
            <select
              ...
              onChange={(e) => {
                setFormData((prev) => ({ ...prev, material: e.target.value as Material }));
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Resumen para tarjetas de catálogo y lookbook..."
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Historia orfebre detallada, inspiración, detalles de confección..."
              className={`w-full px-3 py-2 text-xs bg-cream-200/50 border rounded-sm focus:outline-none focus:border-gold text-ink leading-relaxed ${
        ...
        <div className="space-y-5 bg-cream p-5 rounded-sm border border-taupe/15 shadow-sm animate-fade-in">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                placeholder="/assets/hero/hero-necklace-rings.png"
          ...
          <div className="pt-2">
            <p className="text-xs font-semibold uppercase tracking-wider text-ink mb-2">
              Recursos Fotográficos de Marca (Presets)
            </p>
              ...
              {BRAND_PRESET_IMAGES.map((preset) => (
                <button
                  ...
                  onClick={() => {
                    setFormData((prev) => ({
                      ...prev,
                      cover_image: preset.url,
                      images: prev.images.includes(preset.url) ? prev.images : [...prev.images, preset.url],
                    }));
                  }}
                  className={`p-2 rounded-sm border text-left text-[11px] flex items-center gap-2 transition-colors ${
                    formData.cover_image === preset.url
                      ? 'border-gold bg-cream-200 font-medium text-ink'
                      : 'border-taupe/20 hover:border-taupe/50 text-taupe-contrast'
                  }`}
                >
                  <img src={preset.url} alt="" className="w-6 h-6 object-cover rounded-sm bg-stone/20" />
      ...
      {activeFormTab === 'specs' && (
        <div className="space-y-4 bg-cream p-5 rounded-sm border border-taupe/15 shadow-sm animate-fade-in">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-ink mb-1">
                Kilataje / Gemas (Carats)
              </label>
              <input
                type="text"
                value={formData.specifications.carats || ''}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    specifications: { ...prev.specifications, carats: e.target.value },
                  }))
                }
                placeholder="Ej. 0.75 ct F-VS1"
                className="w-full px-3 py-2 text-xs bg-cream-200/50 border border-taupe/20 rounded-sm focus:outline-none focus:border-gold text-ink"
              />
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Ej. Cadena 45 cm; Dije 6.5 mm"
                className="w-full px-3 py-2 text-xs bg-cream-200/50 border border-taupe/20 rounded-sm focus:outline-none focus:border-gold text-ink"
              />
            </div>
            ...
            <div>
              <label className="block text-xs font-semibold uppercase tracking-wider text-ink mb-1">
                Piedra Principal (Stone)
              </label>
              <input
                type="text"
                value={formData.specifications.stone || ''}
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    specifications: { ...prev.specifications, stone: e.target.value },
                  }))
                }
                placeholder="Ej. Diamante natural corte brillante redondo"
                className="w-full px-3 py-2 text-xs bg-cream-200/50 border border-taupe/20 rounded-sm focus:outline-none focus:border-gold text-ink"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                placeholder="Ej. Oro Ley 750 (18 Kilates)"
                className="w-full px-3 py-2 text-xs bg-cream-200/50 border border-taupe/20 rounded-sm focus:outline-none focus:border-gold text-ink"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData((prev) => ({
                    ...prev,
                    specifications: { ...prev.specifications, closure: e.target.value },
                  }))
                }
                placeholder="Ej. Cierre mosquetón de seguridad"
                className="w-full px-3 py-2 text-xs bg-cream-200/50 border border-taupe/20 rounded-sm focus:outline-none focus:border-gold text-ink"
              />
            </div>
          </div>
