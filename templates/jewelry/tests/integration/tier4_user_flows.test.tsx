...
describe('Tier 4: Real-World User Scenarios (End-to-End)', () => {
  ...
  it('T4-UF-01: VIP Shopper: Landing -> Hero CTA -> Lookbook inspect -> Swipe -> Zoom -> WhatsApp CTA', () => {
    ...
    const waCTA = screen.getByRole('link', { name: new RegExp(`Consultar pieza ${PRODUCTS[0].name}`, 'i') });
  ...
  it('T4-UF-02: Bridal Collection Curation: Search -> Multi-Favorite -> Bulk WhatsApp Inquiry', () => {
    ...
    fireEvent.change(searchInput, { target: { value: 'Anillo' } });
    ...
    // 4. Clear search and search for "Gargantilla"
    fireEvent.change(searchInput, { target: { value: 'Gargantilla' } });
