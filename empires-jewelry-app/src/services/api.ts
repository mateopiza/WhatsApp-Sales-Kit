...
export const CATALOG_STORAGE_KEY = 'empires_jewelry_catalog';
export const ADMIN_AUTH_STORAGE_KEY = 'empires_admin_auth';
...
class ApiService {
  ...
  async getProductById(idOrSlug: string): Promise<ApiResponse<Product>> {
    ...
    return {
      success: false,
      error: `Pieza '${idOrSlug}' no encontrada`,
    };
  }
  ...
  async createProduct(payload: CreateProductPayload, token?: string): Promise<ApiResponse<Product>> {
    ...
    try {
      ...
      const id = payload.id || `emp-${Date.now().toString(36)}`;
  ...
  async updateProduct(id: string, updates: UpdateProductPayload, token?: string): Promise<ApiResponse<Product>> {
    ...
    try {
      ...
      if (!current) {
        return { success: false, error: `Pieza '${id}' no encontrada en caché local` };
      }
