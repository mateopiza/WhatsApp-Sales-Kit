  ...
  describe('Rapid Continuous Trigger Stress Harness', () => {
    ...
    it('CH2-STRESS-04: Admin Portal open/close stress cycles cleanly unmount and clear credentials state', () => {
      ...
      for (let i = 0; i < 50; i++) {
        ...
        expect(screen.getByRole('dialog', { name: /Empires Jewelry CMS/i })).toBeInTheDocument();
        fireEvent.click(closeAdminBtn);
        expect(screen.queryByRole('dialog', { name: /Empires Jewelry CMS/i })).not.toBeInTheDocument();
      }
    });
    ...
    it('CH2-STRESS-05: Favorites Drawer rapid open, add, remove, and close cycles maintain deterministic state', () => {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        expect(screen.getByRole('dialog', { name: /Selección privada de joyas guardadas/i })).toBeInTheDocument();
        fireEvent.click(closeFavsBtn);
        expect(screen.queryByRole('dialog', { name: /Selección privada de joyas guardadas/i })).not.toBeInTheDocument();
      }
    });
  });
});
