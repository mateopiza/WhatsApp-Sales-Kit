...
describe('Empirical Stress & Boundary Challenger Suite', () => {
  ...
  describe('2. Live Search & Filter Engine Edge Cases & Stability', () => {
    ...
    it('SRCH-04: Case-insensitivity across exact reference, material, and full metadata', () => {
      ...
      act(() => {
        result.current.setSearchQuery('REF-0101');
      });
      expect(result.current.filteredProducts.length).toBe(1);
      expect(result.current.filteredProducts[0].id).toBe('prod-01');
      ...
      // Substring shared reference lookup 'REF-010' matches all 6 demo products
      act(() => {
        result.current.setSearchQuery('REF-010');
      });
      expect(result.current.filteredProducts.length).toBe(6);
      ...
      expect(
        result.current.filteredProducts.every(
          (p) =>
            p.material.toLowerCase().includes('premium') ||
            p.description.toLowerCase().includes('premium') ||
            p.short_description.toLowerCase().includes('premium') ||
    ...
    it('SRCH-05: Accent sensitivity / normalization behavior check', () => {
      ...
      // Search with exact accent 'Edición' matches prod-04 (Edición Limitada collection)
      act(() => {
        result.current.setSearchQuery('Edición');
      });
      const accentedCount = result.current.filteredProducts.length;
      expect(accentedCount).toBe(1);
      expect(result.current.filteredProducts.map((p) => p.id)).toEqual(['prod-04']);

      // Search without accent 'Edicion' also matches prod-04 via diacritic normalizer
      act(() => {
        result.current.setSearchQuery('Edicion');
      });
