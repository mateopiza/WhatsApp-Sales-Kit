...
describe('Empirical Challenger 2: User Flow & Performance Adversarial Harness', () => {
  ...
  describe('Dimension 2: Full Luxury User Journey Simulation', () => {
    it('CH-FLOW-01: Hero Cover -> Explore CTA -> Category Navigation -> Product Inspection -> Gestures -> WhatsApp link', async () => {
      ...
      const heroHeading = screen.getByRole('heading', {
        level: 1,
        name: /Vende Directo Por WhatsApp/i,
      });
      expect(heroHeading).toBeInTheDocument();

      const heroBadge = screen.getByText(/Powered by WhatsApp Sales Kit/i);
      ...
      // Step 3: Category Filtering (Select 'Categoría 1')
      const categoryFilter = screen.getByRole('button', { name: 'Categoría 1' });
      fireEvent.click(categoryFilter);

      // Verify filtered catalog shows only the 2 Categoría 1 products
      expect(screen.getByText(/Todas \(2\)/i)).toBeInTheDocument();

      // Step 4: Open Product Viewer for Producto Demo 1 (prod-01)
      ...
      expect(screen.getByRole('heading', { level: 1, name: /Producto Demo 1/i })).toBeInTheDocument();
      ...
      expect(screen.getByAltText(/Producto Demo 1 - Imagen 2/i)).toBeInTheDocument();
      ...
      const waLink = screen.getByRole('link', {
        name: /Consultar Producto Demo 1 por WhatsApp/i,
      });
      ...
      expect(decodedText).toContain('Producto Demo 1');
      expect(decodedText).toContain('REF-0101');
      expect(decodedText).toContain('$49');
    });
  });
  ...
  describe('Dimension 3: Multi-Item Curation & Bulk WhatsApp Generator', () => {
    it('CH-CURATION-01: Multi-item curation across multiple categories generates robust bulk WhatsApp payload', () => {
      const selectedProducts = [PRODUCTS[0], PRODUCTS[1], PRODUCTS[2], PRODUCTS[3]]; // prod-01, prod-02, prod-03, prod-04
      ...
      expect(message).toContain(`2. *${PRODUCTS[1].name}*`);
      expect(message).toContain(`3. *${PRODUCTS[2].name}*`);
      expect(message).toContain(`4. *${PRODUCTS[3].name}*`);

      // Total expected: 49 + 89 + 129 + 199 = $466
      expect(message).toContain('Total estimado: $466 (4 productos)');
    });
    ...
    it('CH-CURATION-02: Favorites Drawer renders curated items, updates total dynamically, and supports removal', () => {
      ...
      const drawer = screen.getByRole('dialog', { name: /Selección privada de productos guardados/i });
      expect(drawer).toBeInTheDocument();

      // Verify items present inside the drawer
      expect(within(drawer).getByText('3 Productos Guardados')).toBeInTheDocument();
      ...
      // Total: 49 + 89 + 35 = $173
      expect(within(drawer).getByText('$173')).toBeInTheDocument();
      ...
      // Now 2 items left, total: 89 + 35 = $124
      expect(within(drawer).getByText('2 Productos Guardados')).toBeInTheDocument();
      expect(within(drawer).getByText('$124')).toBeInTheDocument();
    });
  });
  ...
  describe('Dimension 4: Web Share & Clipboard Fallback Oracle', () => {
    const testShareData = {
      title: 'Producto Demo 1 — My Store',
      text: 'Descubre Producto Demo 1 (REF-0101) en My Store.',
      url: 'https://example.com/#producto-demo-uno',
    };
  ...
  describe('Dimension 5: PWA Install Hook & Offline Shell Integrity', () => {
    ...
    it('CH-PWA-02: sw.js defines cache headers and responds with fallback image SVG on offline network error', () => {
      ...
      expect(swSource).toContain('store-static-v1');
      expect(swSource).toContain('store-shell-v1');
      expect(swSource).toContain('MAX_IMAGE_CACHE_ENTRIES = 60');
      expect(swSource).toContain('image/svg+xml');
      expect(swSource).toContain('SIN CONEXIÓN');
    });
  });
